...
class AutoTuneWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.target_combo.addItems(["Balanced", "Max Speed", "Low VRAM", "Quality KV", "MoE Optimized"])
        ...
        self.activity_log.setStyleSheet("background-color: #1e1e1e; color: #d4d4d4; font-family: Consolas;")
    ...
    def set_running(self, running: bool) -> None:
        ...
        self.apply_best_btn.setEnabled(False if running else self.apply_best_btn.isEnabled())
        self.save_best_btn.setEnabled(False if running else self.save_best_btn.isEnabled())
    ...
    def _set_item(self, row: int, col: int, value: object) -> None:
        ...
        item.setTextAlignment(Qt.AlignmentFlag.AlignCenter if col != len(_COLUMNS) - 1 else Qt.AlignmentFlag.AlignLeft)
    ...
    def mark_running(self, candidate: BenchmarkCandidate) -> None:
        ...
        self.current_run_label.setText(
            ...
            f"b={p.get('batch_size')} ub={p.get('ubatch_size')} | "
            f"t={p.get('threads')} ncmoe={p.get('ncmoe')}"
        )
        self.append_activity(
            f"START {candidate.id}: KV {p.get('cache_type_k')}/{p.get('cache_type_v')}, "
    ...
    def update_result(self, result: BenchmarkResult) -> None:
        ...
        values = {
            1: result.status,
            2: f"{result.score:.3f}",
            3: f"{result.prompt_tok_s:.1f}",
            4: f"{result.generation_tok_s:.1f}",
            5: f"{result.load_time_sec:.1f}",
            6: f"{result.vram_used_mib:.0f}",
            7: f"{result.ram_used_mib:.0f}",
            21: result.error,
        }
    ...
    def show_best(self, best: Optional[BenchmarkResult], params: Dict[str, object], output_dir: str) -> None:
        self._timer.stop()
        self.progress_bar.setRange(0, max(self._total_runs, 1))
        self.progress_bar.setValue(self._total_runs if best else self._done_runs)
        self.progress_bar.setFormat(f"{self.progress_bar.value()}/{max(self._total_runs, 1)} runs")
        self.current_run_label.setText(f"Finished. Results folder: {output_dir}" if output_dir else "Finished")
        self._update_time_labels(finished=True)
        self.export_report_btn.setEnabled(bool(output_dir))
        self.open_results_btn.setEnabled(bool(output_dir))
        if not best:
            self.best_text.setPlainText(f"No successful result. Results folder: {output_dir}")
            self.apply_best_btn.setEnabled(False)
            self.save_best_btn.setEnabled(False)
            return
    ...
    def _update_time_labels(self, finished: bool = False) -> None:
        ...
        current_elapsed = now - self._current_run_started_at if self._current_run_started_at else 0
        ...
        if self._done_runs > 0 and total_elapsed > 0:
            ...
            eta_text = f"up to {self._format_duration(self._per_run_timeout_sec * remaining)}"
        else:
            eta_text = "estimating"
